/*
[Fork by https://github.com/windycom/leaflet-kml/tree/master]
...
L.Util.extend(L.KML, {
	...
	parseStyles: function (xml, kmlOptions) {
		var styles = {};
		var sl = xml.getElementsByTagName('gx:CascadingStyle'); //ex Style
	...
	parseStyle: function (xml, kmlOptions) {
		...
		id = xml.getAttribute('kml:id'); //ex id
		if (id && style) {
			style.id = id;
		}
...
L.KMLIcon = L.Icon.extend({
	...
	applyCustomStyles: function(img) {
		...
		this.options.popupAnchor = [0,(-0.83*height)];
        	...
        	//insetPixels
        	if (options.anchorType.x === 'insetPixels')
        		img.style.marginLeft = (-options.anchorRef.x / 2) + 'px';
        	if (options.anchorType.y === 'insetPixels')
        		img.style.marginTop = (options.anchorRef.y - height + 1) + 'px';
         	*/
	}
});
